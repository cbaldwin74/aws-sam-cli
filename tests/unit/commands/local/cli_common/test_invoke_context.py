...
class TestInvokeContext__enter__(TestCase):
    ...
    @patch("samcli.commands.local.cli_common.invoke_context.SamBuildableStackProvider.get_buildable_stacks_recursively")
    def test_must_raise_if_template_cannot_be_parsed(self, get_buildable_stacks_recursively_mock):
        invoke_context = InvokeContext("template-file")

        get_buildable_stacks_recursively_mock.side_effect = TemplateFailedParsingException("")
        with self.assertRaises(InvokeContextException) as ex_ctx:
            invoke_context.__enter__()
