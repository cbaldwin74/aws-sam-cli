...
from .provider import ServerlessApplication
...
class SamServerlessApplicationProvider(SamBaseProvider):
    ...
    It may or may not contain a serverless application.
    """
    ...
    def __init__(self, template_dict, parameter_overrides=None, ignore_code_extraction_warnings=False):
        ...
        self.applications = self._extract_serverless_applications(self.resources, self.ignore_code_extraction_warnings)

        LOG.debug("%d serverless application resources found in the template", len(self.applications))
    ...
    def _extract_serverless_applications(resources, ignore_code_extraction_warnings=False):
        ...
        :return dict(string : samcli.commands.local.lib.provider.ServerlessFunction): Dictionary of application
            LogicalId to the ServerlessApplication configuration object
        """
    ...
    def _convert_sam_serverless_application_resource(name, resource_properties, ignore_code_extraction_warnings=False):
        """
        Converts a AWS::Serverless::Application resource to a ServerlessApplication configuration usable by the
        provider.
    ...
    def _build_serverless_application_configuration(
        ...
        return ServerlessApplication(
            name=name,
            location=location,
            notification_arns=resource_properties.get("NotificationARNs"),
