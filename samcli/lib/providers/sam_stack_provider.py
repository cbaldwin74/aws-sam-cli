...
class SamBuildableStackProvider(SamBaseProvider):
    ...
    @staticmethod
    def get_buildable_stacks_recursively(
        template_file: str, stack_path: str = "", name: str = "", parameter_overrides: Optional[Dict] = None
    ) -> List[BuildableStack]:
        ...
        for child_stack in current.get_all():
            stacks.extend(
                SamBuildableStackProvider.get_buildable_stacks_recursively(
                    child_stack.location, os.path.join(stack_path, name), child_stack.name, child_stack.parameters
                )
            )
        return stacks
